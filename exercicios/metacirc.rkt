...
;; avalia uma lista de expressões, retornando o valor da ultima
...
(adiciona-primitiva '<= <=)
(adiciona-primitiva '>= >=)
(adiciona-primitiva 'sub1 sub1)

;;primitivas para string
(adiciona-primitiva 'length string-length)
(adiciona-primitiva 'concat string-append)

;;primitivas para listas
(adiciona-primitiva 'cons cons)
(adiciona-primitiva 'first first)
(adiciona-primitiva 'rest rest)

( define prog-fat-1
   '(begin
      (define fat (lambda (n) (if (= n 0)
                                 1
                                 (* n (fat (sub1 n))))))
   (fat 5)))
...
(module+ test
  ...
  (define-test-suite test-eval
    ...
    (test-equal? "begin vazio" (eval '(begin)) (void))
    (test-equal? "begin com uma operação" (eval '(begin (+ 3 2))) 5)
    (test-equal? "begin com duas operações" (eval '(begin (+ 3 2) (* 4 8))) 32)
    (test-equal? "begin iniciando variável" (eval '(begin (define x 2) x)) 2) 
    (test-exn "variavel nao definida"
              exn:fail?
              (lambda()
                (eval-temp-env '()
                                '(begin (define x 2) (+ y 4)))))
