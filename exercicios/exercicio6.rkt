...
(define (filtra-par-lg lst)
  (filter (lambda (x) (and (integer? x) (even? x)))
          lst))                                        ;; lst pode conter valores de qualquer tipo
...
(define (menor-quociente x lst)
  (define quocientes (map(lambda (y) (/ x y))
         (filter ( lambda (n) (not (zero? n))) lst)))
  (foldl (lambda (a b) (min a b))
         (first quocientes)
         quocientes))
